...
export function attendeeEmail(event: CalWebhookPayload): string {
  const primary = event.payload?.attendees?.[0]?.email ?? "";
  const fallback = valueOfResponse(event.payload?.responses?.email);
  return (primary || fallback).trim().toLowerCase();
}

function attendeeName(event: CalWebhookPayload): { firstName?: string; lastName?: string } {
  const raw = event.payload?.attendees?.[0]?.name || valueOfResponse(event.payload?.responses?.name);
...
async function findOrCreateContact(client: CalFunnelrClient, event: CalWebhookPayload, email: string): Promise<FunnelrUser> {
  const existing = await client.findContactByEmail(email);
  if (existing) return existing;
  return client.createContact({ email, ...attendeeName(event) });
}
...
export async function handleCalWebhook(
  ...
  if (event.payload?.type !== expectedSlug) {
  ...
  try {
    if (event.triggerEvent === "BOOKING_CREATED" || event.triggerEvent === "BOOKING_RESCHEDULED") {
      const contact = await findOrCreateContact(client, event, email);
